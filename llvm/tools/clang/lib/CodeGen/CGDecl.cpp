...
void CodeGenFunction::EmitDecl(const Decl &D) {
	...
	std::cout << "Emiting decl!" << std::endl;
  switch (D.getKind()) {
...
void CodeGenFunction::EmitVarDecl(const VarDecl &D) {
  ...
  //FIXME Carson (this is right :D )
  std::string test_str = D.getType().getAsString();
  std::cerr << test_str << std::endl;

  //FIXME Carson maybe make MD here with type, size, etc.
...
CodeGenFunction::EmitAutoVarAlloca(const VarDecl &D) {
  ...
  if (Ty->isConstantSizeType()) {
    ...
    if (getLangOpts().OpenMP && OpenMPLocalAddr.isValid()) {
      ...
      // builds.
      allocaTy->type_info = new std::string("TESTING TYPE");
